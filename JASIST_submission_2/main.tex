%% ACM Large 1-Column Format (acmlarge)
%% Journal of the Association for Information Science and Technology (JASIST)
\documentclass[acmlarge]{acmart}

%% Use author-year citation style (natbib-compatible)
\citestyle{acmauthoryear}

%% Rights / journal metadata (fill in upon acceptance)
\setcopyright{rightsretained}
\copyrightyear{2026}
\acmYear{2026}
\acmDOI{XXXXXXX.XXXXXXX}

%% Journal fields (fill in upon acceptance — JASIST is not an ACM journal so these
%% are left blank; the acmlarge class option is used for its single-column formatting)
%\acmJournal{JACM}   % uncomment and replace with correct journal if submitting to ACM
%\acmVolume{0}
%\acmNumber{0}
%\acmArticle{0}
%\acmMonth{0}

%% Additional packages (natbib, hyperref, fontenc, geometry already in acmart.cls)
\usepackage{amsmath}
...
%% Custom commands
\newcommand{\paratitle}[1]{\par\smallskip\noindent\textbf{#1}}
...
Evaluating LLM Adaptation to Sociodemographic Factors:\texorpdfstring{\\}{ }%
User Profile vs.\ Dialogue History}

\author{Qishuai Zhong}
\authornote{Corresponding author.}
\email{qishuai.zhong@ntu.edu.sg}
\affiliation{%
...
%%
%% CCS Concepts (replace with correct terms from https://dl.acm.org/ccs/ccs.cfm)
\begin{CCSXML}
<ccs2012>
 <concept>
...
%%
%% Keywords
\keywords{large language models, sociodemographic adaptation, value alignment, individual preservation, behavioral evaluation}

\received{[Submission date]}
\received[revised]{[Revision date]}
\received[accepted]{[Acceptance date]}
...
\begin{figure}[H]
\begin{center}
    \includegraphics[trim=0.5cm 4.5cm 6.5cm 1cm,clip=true,width=0.75\textwidth]{figures/PersonaExampleWide.pdf}
    \Description{Diagram showing the evaluation framework with two input formats: explicit user profiles (top) and dialogue history (bottom), feeding into LLM responses and evaluation criteria for adaptation, alignment, preservation, and consistency.}
